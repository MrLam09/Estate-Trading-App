...
const List1 = () => {
    ...
    const {data: estatesinf = []} = useFetchAllEstateQuery();
    

    const findRegions = region === 'Khu vực' ? estatesinf 
    : estatesinf.filter(estatesinf => estatesinf.location === region);
    ...
    return (
        <div>
            <h2 className='mb-6 text-3xl font-semibold'>Danh sách BĐS</h2>
                ...
                >
            {
                findRegions.length > 0 && findRegions.map((estatesinf, index) => (
                    <SwiperSlide key={index}>
                        <EsInf estate={estatesinf}/>
                    </SwiperSlide>
                ))
            }
